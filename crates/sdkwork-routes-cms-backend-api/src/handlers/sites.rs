...
pub async fn list_sites(
    ...
    match service.list_sites(ctx, query).await {
        Ok(page) => {
            let response =
                res_mapper::map_page_to_paginated_response(page, res_mapper::map_site_to_response);
...
pub async fn create_site(
    ...
    match service.create_site(ctx, command).await {
        Ok(site) => ApiResponse::success(
            res_mapper::map_site_to_response(site),
            Some(ctx.request_id.clone()),
        ),
        Err(err) => ApiResponse::error(
            problem::map_cms_error_to_problem(&err, ctx.trace_id.clone()),
    ...
    match service.list_channels(ctx, query).await {
        Ok(page) => {
            let response = res_mapper::map_page_to_paginated_response(
                page,
                res_mapper::map_channel_to_response,
            );
            ApiResponse::success(response, Some(ctx.request_id.clone()))
        }
        Err(err) => ApiResponse::error(
...
pub async fn create_channel(
    ...
    match service.create_channel(ctx, command).await {
        Ok(channel) => ApiResponse::success(
            res_mapper::map_channel_to_response(channel),
            Some(ctx.request_id.clone()),
        ),
        Err(err) => ApiResponse::error(
            problem::map_cms_error_to_problem(&err, ctx.trace_id.clone()),
