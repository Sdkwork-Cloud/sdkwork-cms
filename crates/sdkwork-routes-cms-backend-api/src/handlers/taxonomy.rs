    ...
    match service.list_taxonomies(ctx, query).await {
        Ok(page) => {
            let response = res_mapper::map_page_to_paginated_response(
                page,
                res_mapper::map_taxonomy_to_response,
            );
...
pub async fn create_taxonomy(
    ...
    match service.create_taxonomy(ctx, command).await {
        Ok(taxonomy) => ApiResponse::success(
            res_mapper::map_taxonomy_to_response(taxonomy),
            Some(ctx.request_id.clone()),
        ),
        Err(err) => ApiResponse::error(
            problem::map_cms_error_to_problem(&err, ctx.trace_id.clone()),
...
pub async fn list_taxonomy_terms(
    ...
    match service.list_taxonomy_terms(ctx, query).await {
        Ok(page) => {
            let response = res_mapper::map_page_to_paginated_response(
                page,
                res_mapper::map_taxonomy_term_to_response,
            );
            ApiResponse::success(response, Some(ctx.request_id.clone()))
        }
        Err(err) => ApiResponse::error(
...
pub async fn create_taxonomy_term(
    ...
    match service.create_taxonomy_term(ctx, command).await {
        Ok(term) => ApiResponse::success(
            res_mapper::map_taxonomy_term_to_response(term),
            Some(ctx.request_id.clone()),
        ),
        Err(err) => ApiResponse::error(
            problem::map_cms_error_to_problem(&err, ctx.trace_id.clone()),
